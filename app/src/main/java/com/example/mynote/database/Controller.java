...
public class Controller {
     ...
     */
    public <T extends DatabaseModel> ArrayList<T> findNotes
    (Class<T> cls, String[] columns, String where, String[] whereParams, int sortId) {
    ...
    public void addCategoryCounter(long categoryId, int amount) {
        ...
        try {
            Cursor c = db.rawQuery(
                    "UPDATE " + OpenHelper.TABLE_NOTES + " SET "
    ...
    public void undoDeletion() {
        ...
        try {
            ...
            if (c != null) {
                while (c.moveToNext()) {
                    String query = c.getString(c.getColumnIndex(
                            OpenHelper.COLUMN_SQL));
                    if (query != null) {
     ...
     */
    public <T extends DatabaseModel> long saveNote
    (T note, ContentValues values) {
        SQLiteDatabase db = helper.getWritableDatabase();
